...
public class HybridPolicy implements ContextualBanditPolicy<User, Article, Boolean> {
	...
	public Article getActionToPerform(User visitor,
		...
		for (Article article: possibleActions) {
			...
			DoubleMatrix zta = xta.mmul(z.get(article)).reshape(SIZE2, 1);
			...
			sta -= 2 * zta_tran.mmul(invA0).mmul(Ba_tran).mmul(invAa).mmul(xta).get(0, 0);
			sta += xta_tran.mmul(invAa).mmul(xta).get(0, 0);
			sta += xta_tran.mmul(invAa).mmul(Ba).mmul(invA0).mmul(Ba_tran).mmul(invAa).mmul(xta).get(0, 0);
	...
	public void updatePolicy(User c, Article a, Boolean reward) {
		...
		DoubleMatrix xta_tran = xta.transpose();
		DoubleMatrix zta = xta.mmul(z.get(a)).reshape(SIZE2, 1);
		DoubleMatrix zta_tran = zta.transpose();
		...
		if (inverseSteps >= INVERSE_STEPS) {
			invA0 = Solve.solve(A0, DoubleMatrix.eye(SIZE));
